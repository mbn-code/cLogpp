...
public:
    ...
    void log(const std::string& line) override {
        const std::size_t need = line.size() + 1;  // + newline
        if (max_bytes_ != 0 && current_size_ > 0 && current_size_ + need > max_bytes_) rotate();
        file_ << line << '\n';
        current_size_ += need;
    }
